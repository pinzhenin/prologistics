<?php
function smarty_function_mulang_edit2($params, &$smarty)
{
	...
	foreach($langs as $lang_id=>$lang) {
		...
		$res .= '		<b>'.$lang.'</b><br><input type="text" id="trnewvalue['.$values[$lang_id]->iid.']" name="'.$field_name.$index.'['.$lang_id.']" value="'.$values[$lang_id]->value.'" size="'.$size.'" style="'.$style.'"'.($limit?' onKeyUp="color(this);" ':'').'>';
		if ($limit) {
			...
			(<input type="text" id="cnt1'.$field_name.'['.$lang_id.']" value="" readonly style="border: 0px;background-color:#eeeeee; text-align:right" size="10">) /
	  ...
	  <script>color(document.getElementsByName("'.$field_name.'['.$lang_id.']").item(0))</script>';
			...
			(<input type="text" value="w prefix, suffix" readonly style="border: 0px;background-color:#eeeeee; text-align:right" size="10">) /
