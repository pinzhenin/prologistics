<?php
function smarty_function_mulang_select2($params, &$smarty)
{
	...
	foreach($langs as $lang_id=>$lang) {
		...
		$res .= '		<b>'.$lang.'</b><br>';
		$res .= '		<select  id="trnewvalue['.$values[$lang_id]->iid.']" name="'.$field_name.'['.$lang_id.']" '.$disabled.($limit?' onChange="colorSelect(this);" ':'').'>';
