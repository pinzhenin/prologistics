...
function smarty_function_html_select_date($params, &$smarty)
{
    ...
    if ($display_months) {
		...
		list($dummy, $locale) = explode('LC_TIME=',$locales);
		list($locale, $dummy) = explode(';',$locale);
        for ($i = 1; $i <= 12; $i++) {
			if ($locale=='pl_PL') {
	            $month_names[] = utf8_encode(strftime($month_format, mktime(0, 0, 0, $i, 1, 2000)));
			} else {
	            $month_names[] = utf8_encode(strftime($month_format, mktime(0, 0, 0, $i, 1, 2000)));
			}
        ...
        $month_result .= '<select '.$function.' '.$style.' name=';
        if (null !== $field_array){
    ...
    if ($display_days) {
        ...
        $day_result .= '<select '.$function.' '.$style.' name=';
        if (null !== $field_array){
    ...
    if ($display_years) {
        if (null !== $field_array){
            $year_name = $field_array . '[' . $prefix . 'Year]'.' id='.$year_name;
        } else {
        ...
        if ($year_as_text) {
            ...
            $year_result .= '<select '.$function.' '.$style.' name="' . $year_name . '"';
            if (null !== $year_size){
