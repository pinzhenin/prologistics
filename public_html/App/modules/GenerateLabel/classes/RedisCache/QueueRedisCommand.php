...
class QueueRedisCommand extends Command
{
    ...
    public function processMessage($level, $message)
    {
        if (
            ...
            file_put_contents(TMP_DIR . '/redis_queue.txt', $record . PHP_EOL, FILE_APPEND);
        }
    }
    ...
    {
        RedisJob::setEmulate(self::$input->getOption(self::OPTION_EMULATE));
        RedisJob::setCallbackMessage(['label\RedisCache\QueueRedisCommand', 'processMessage']);
    ...
    {
        $command = 'php console.php queue:redis';
        ...
        exec('ps auxwww|grep "resque"|grep "redis"|grep -v grep|grep -v php-fpm| grep -v bash|grep -v "su -"', $res);
        $n = $n - count($res);
