...
#		        , null as username
		  ...
		  FROM orders o
			left join total_log_stock tl on tl.tableid=o.id and table_name='orders' and field_name='sent' and New_value=1
...
SELECT 
			  	rs.article_id
				, total_log_stock.updated as date
				, CONCAT(rp.name, '# ', r.rma_id) as comment
				, 1 as quantity
				, null as id
				, IFNULL(users.name,total_log_stock.username) as user
				, r.rma_id as rma_id
				, null as order_id
				, 'Wrong' as TYPE
			, r.auction_number
...
#		        , IFNULL( users.name, r.responsible_uname ) as username
		  ...
		  FROM rma r
			...
			left join (select 
				...
				, TableID 
				from total_log_stock where 1
				and Table_name='rma_spec'
				and Field_name='problem_id'
				and New_value in ('4','11')
				group by TableID
			) total_log_stock on 1 and TableID=rs.rma_spec_id
			left join users on users.system_username=total_log_stock.username
...
#		        , IFNULL( users.name, rs.responsible_uname ) as username
		  ...
		  FROM rma r
			JOIN rma_spec rs ON r.rma_id=rs.rma_id
			left join total_log_stock tl on tl.table_name='rma_spec' and tl.field_name='add_to_stock' and tl.tableid=rs.rma_spec_id and tl.new_value='1'
			LEFT JOIN users ON rs.responsible_uname=users.username 
			left JOIN warehouse w ON rs.warehouse_id = w.warehouse_id
			LEFT JOIN auction ON r.auction_number=auction.auction_number and r.txnid=auction.txnid
...
SELECT 
			  	wwa.article_id
				, (select tl.updated from total_log_stock tl
				...
				, -wwa.qnt as quantity
				, wwo.id as id
				, (select u.username from total_log_stock tl
				left join users u on u.system_username=tl.username
				where table_name='wwo_article' and field_name='taken' and tableid=wwa.id
...
SELECT 
				...
				, o.new_article_qnt
				, null as id
				, (select IFNULL(users.name, total_log_stock.username)
					from total_log_stock
					left join users on users.system_username=total_log_stock.username
					where table_name='orders' and Field_name='new_article'
...
SELECT 
		  ...
		  FROM orders o
			...
			AND o.txnid = au.txnid
					...
					left join auction_par_varchar au_city_shipping on au.auction_number=au_city_shipping.auction_number 
						and au.txnid=au_city_shipping.txnid and au_city_shipping.key='city_shipping'
			left JOIN warehouse w ON IF(wwa_unc.delivered, wwa_unc.to_warehouse, IF(wwa_unc.taken, w_driver.warehouse_id, o.new_article_warehouse_id)) = w.warehouse_id
...
from orders o2 where o2.uncomplete_article_order_id=o.id)),'')
		  ...
		  FROM orders o
		    ...
		    left join vbarcode_warehouse1 bw ON bw.id = bo.barcode_id
        ...
        foreach($r as $kk=>$row) {
			if (strlen($row->container_ids) || strlen($row->container_ids1)) {
				$q = "select opc.*
					...
					where el.template='op_container_delivery_document' and el.notes*1=opc.id
					and el.auction_number=opc.order_id and el.txnid=-1
					and el.recipient like CONCAT('%',wc.email,'%')
			...
			left join op_order_container opc on opc.id = IFNULL(master.id, op_order_container.id)
				...
				where opc.id in ({$row->container_ids},{$row->container_ids1})
					ORDER BY id";
//				echo '<br>'.nl2br($q);
...
#			echo $q.'<br>';
    ...
    static function getSoldCountDay($db, $dbr, $article_id, $days, $cons = '', $warehouses=array()){
		...
		(select IFNULL((select SUM(o.quantity)
			...
			JOIN auction au ON o.auction_number = au.auction_number
				AND o.txnid = au.txnid
			left join total_log_stock on total_log_stock.table_name='orders' and total_log_stock.field_name='sent' 
				and total_log_stock.tableid=o.id and total_log_stock.New_value='1'
			WHERE $art_cond and o.manual=0
			AND total_log_stock.updated >= DATE_SUB(NOW(), INTERVAL $days DAY)
    ...
    static function getSoldCountMonth($db, $dbr, $article_id, $mnth, $cons = ''){
        ...
        $r = $dbr->getOne("(select IFNULL((select SUM(o.quantity)
			...
			left join total_log_stock on total_log_stock.table_name='orders' and total_log_stock.field_name='sent' 
				and total_log_stock.tableid=o.id and total_log_stock.New_value='1'
			WHERE $art_cond and o.manual=0
			AND total_log_stock.updated >= DATE_SUB(NOW(), INTERVAL $mnth MONTH)
			AND au.deleted = 0),0))");
        if (PEAR::isError($r)) {
			aprint_r($r);
            return;
    ...
    static function getPeriodSales($db, $dbr, $days, $article_id){
        $r = $dbr->getAssoc("select FROM_DAYS( TO_DAYS( total_log_stock.updated ) ) AS date, SUM(o.quantity)
			FROM orders o
			JOIN auction au ON o.auction_number = au.auction_number
				AND o.txnid = au.txnid
			left join total_log_stock on total_log_stock.table_name='orders' and total_log_stock.field_name='sent' 
			...
			AND total_log_stock.updated >= DATE_SUB(NOW(), INTERVAL $days DAY)
			AND au.deleted = 0
			GROUP BY TO_DAYS( total_log_stock.updated )");
        if (PEAR::isError($r)) {
			aprint_r($r);
            return;
        }
